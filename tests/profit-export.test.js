...
const requiredSheets = ['利润摘要', '输入参数', '成本明细', '盈亏平衡分析', '情景分析', '敏感性分析', '数据口径'];
for (const [marketplace, currency, exchangeRate] of [['US', 'USD', 7.12], ['UK', 'GBP', 9.36], ['DE', 'EUR', 8.42]]) {
  ...
  assert.equal(row('Exchange Rate')[1], exchangeRate, `${marketplace} exchange-rate snapshot`);
  assert.equal(row('SKU')[1], 'DG-001', `${marketplace} SKU`);
  assert.equal(row('备注')[1], '第一批测试，目标英国站。\n包含中文换行。', `${marketplace} Chinese notes`);
...
assert.equal(bufferRow('Exchange Rate Mode')[1], '自动', 'automatic exchange-rate mode');
assert.equal(bufferRow('Exchange Rate Status')[1], '缓存', 'cached exchange-rate status');
